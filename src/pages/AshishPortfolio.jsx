...
const ASHISH_USER_ID = '61f54b73-07f2-4178-bac2-d1412fba9e62'

export default function AshishPortfolio() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const res = await fetch(`${API_BASE}/portfolio/user/${ASHISH_USER_ID}`)
        if (!res.ok) {
  ...
  // Animate hero image -> left of description (same diagonal animation as PublicPortfolio)
  useLayoutEffect(() => {
  ...
  const handleSendMessage = async e => {
    ...
    if (!contactEmail || !contactMessage) return setContactFeedback({ type: 'error', text: 'Please fill all fields' })

    const emailOk = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contactEmail)
    if (!emailOk) return setContactFeedback({ type: 'error', text: 'Invalid email address' })

    const receiver_id = portfolio?.about?.user_id || portfolio?.about?.id || ASHISH_USER_ID
      ...
      <div className="w-full" style={{ marginTop: `-${NAVBAR_HEIGHT}px` }}>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              <div className="text-white space-y-6">
                <h1 className="text-5xl md:text-7xl font-bold gradient-text animate-fade-in">
                  {portfolio.about.name || 'Ashish Biswas'}
                </h1>
                {portfolio.about.designation && (
                  <p className="text-sm md:text-base text-purple-300 font-medium animate-fade-in" style={{animationDelay: '0.2s'}}>{portfolio.about.designation}</p>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {portfolio.skills.length > 0 ? (
                portfolio.skills.map((skill, i) => (
                  <div key={i} className="glass-effect p-6 rounded-xl text-center transition-all duration-300 hover:scale-105 hover:shadow-xl">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {portfolio.projects.length > 0 ? (
                portfolio.projects.map((project, i) => (
                  <div key={i} className="bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                    {project.image_url && (
                    ...
                    >
                      {portfolio.experience.map((exp, i) => (
                        <div key={i} className="min-w-full flex-shrink-0 px-2">
                          <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-500 overflow-hidden border border-gray-100">
                            <div className="p-6">
                  ...
                  {portfolio.experience.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 bg-white hover:bg-purple-50 disabled:opacity-30 disabled:cursor-not-allowed text-gray-800 rounded-full p-3 shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-300 z-10"
                      ...
                      <button
                        ...
                        className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 bg-white hover:bg-purple-50 disabled:opacity-30 disabled:cursor-not-allowed text-gray-800 rounded-full p-3 shadow-lg hover:shadow-xl hover:scale-110 transition-all duration-300 z-10"
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {portfolio.services && portfolio.services.length > 0 ? (
                portfolio.services.map((service, i) => (
                  <div key={i} className="bg-gradient-to-br from-purple-50 to-white rounded-xl shadow-lg p-6 border border-purple-100 transition-all duration-300 hover:scale-105 hover:shadow-xl">
        ...
        <section id="contact-owner" className="section-padding bg-gray-50">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-center text-gray-600 mb-6">Have a question or want to work together? Send me a message!</p>
